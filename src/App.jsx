...
const TIER_CONFIG = {
  free: {
    patternCap: 5,
    // All features available until cap is hit — then imports lock
    priceLabel: "Free",
  },
  pro: {
    patternCap: Infinity,
...
/*
  FREE TIER LOGIC (corrected):
  - Under 5 patterns: ALL features available including all import methods
  - At 5 patterns:    Cannot add more; must upgrade to continue
  - Pro:              Unlimited everything always

  isPro=false + patternCount < 5  → full access
  isPro=false + patternCount >= 5 → blocked, upgrade required
  isPro=true                      → full access always
*/
const useTier = (isPro, patternCount) => {
  ...
  // All features open unless capped
...
/* ══════════════════════════════════════════════════════════════════════════
   PAYWALL GATE — shown only when user is AT cap and tries to add more
══════════════════════════════════════════════════════════════════════════ */
  ...
  <div style={{position:"fixed",inset:0,zIndex:500,display:"flex",alignItems:"flex-end"}} onClick={onClose}>
    ...
    <div className="su" onClick={e=>e.stopPropagation()}
      ...
      <div style={{background:`linear-gradient(135deg,${T.terra},#7A2E14)`,borderRadius:18,padding:"20px",marginBottom:16}}>
        ...
        {["Unlimited pattern storage","All import methods — URL, PDF, photo, browser","AI pattern completion & recreation","Full calculator suite","Cloud sync across all devices"].map(f=>(
...
const URLImportForm = ({onSave}) => {
  ...
  const doImport = () => {
    ...
    setLoading(true);
    setTimeout(()=>{
      setPreview({title:"Imported Pattern",source:url.replace(/^https?:\/\//,"").split("/")[0],cat:"Uncategorized",hook:"",weight:"",yardage:0,notes:"Imported via URL. AI checked for completeness.",materials:[],rows:[],photo:PILL[Math.floor(Math.random()*PILL.length)],aiNote:"Pattern fully scraped — no AI completion needed."});
      setLoading(false);
    },1400);
  };
  return (
    <div style={{paddingBottom:8}}>
      <div style={{fontSize:13,color:T.ink2,lineHeight:1.7,marginBottom:14}}>Paste a pattern URL. We scrape the details automatically and use AI to fill any gaps.</div>
      ...
      {preview&&!loading&&(
        <div className="fu" style={{background:T.linen,borderRadius:16,overflow:"hidden",border:`1px solid ${T.border}`}}>
          ...
          <div style={{padding:"14px"}}>
            ...
            {preview.aiNote&&<div style={{background:T.sageLt,borderRadius:8,padding:"8px 12px",marginBottom:12,display:"flex",gap:8}}><span>🤖</span><span style={{fontSize:12,color:T.sage}}>{preview.aiNote}</span></div>}
...
const PhotoImportForm = ({onSave}) => {
  ...
  const handleFile = (e) => {
    ...
    setLoading(true);
    setTimeout(()=>{
      setPreview({title:"AI-Generated Pattern",source:"Photo Import",cat:"Uncategorized",hook:"5.0mm",weight:"Worsted",yardage:800,notes:"Pattern recreated by AI from photograph. Review rows before starting.",materials:[{id:1,name:"Worsted weight yarn",amount:"~800 yds",yardage:800},{id:2,name:"5.0mm crochet hook",amount:"1"}],rows:[{id:1,text:"Foundation: Magic ring or Ch 4, join",done:false,note:""},{id:2,text:"Rnd 1: 6 sc in ring (6 sts)",done:false,note:""},{id:3,text:"Rnd 2: Inc in each st (12 sts)",done:false,note:""},{id:4,text:"Continue as AI suggests — review carefully",done:false,note:""}],aiNote:"AI analyzed your photo and generated a starter pattern. Adjust as needed."});
      setLoading(false);
    },2000);
  };
  return (
    <div style={{paddingBottom:8}}>
      <div style={{background:`linear-gradient(135deg,${T.terraLt},#FFF8F5)`,borderRadius:12,padding:"12px 14px",marginBottom:14,border:`1px solid ${T.border}`}}>
        ...
        <div style={{fontSize:12,color:T.ink2,lineHeight:1.6}}>Photograph any finished crochet object. AI generates a starter pattern to recreate it.</div>
      </div>
      {!file&&(
        <label style={{display:"block",cursor:"pointer"}}>
          <div style={{border:`2px dashed ${T.border}`,borderRadius:16,padding:"36px 20px",textAlign:"center",background:T.linen}}>
            ...
            <div style={{fontSize:13,color:T.ink3,marginBottom:14}}>Any finished crochet object</div>
      ...
      {imgSrc&&loading&&(
        <div style={{textAlign:"center",padding:"16px 0"}}>
          <div style={{width:"100%",height:150,borderRadius:14,overflow:"hidden",marginBottom:14,position:"relative"}}>
            ...
            <div style={{position:"absolute",inset:0,background:"rgba(184,90,60,.35)",display:"flex",alignItems:"center",justifyContent:"center",backdropFilter:"blur(2px)"}}>
              <div style={{textAlign:"center"}}><div style={{fontSize:28,marginBottom:6}}>🤖</div><div style={{fontSize:13,color:"#fff",fontWeight:600}}>Gemini is analyzing…</div></div>
            </div>
          </div>
          <div style={{fontSize:13,color:T.ink3}}>Identifying stitch patterns and construction</div>
        </div>
      )}
      {preview&&!loading&&(
        <div className="fu" style={{background:T.linen,borderRadius:16,border:`1px solid ${T.border}`,overflow:"hidden"}}>
          {imgSrc&&<div style={{height:110,overflow:"hidden"}}><img src={imgSrc} alt="source" style={{width:"100%",height:"100%",objectFit:"cover"}}/></div>}
          <div style={{padding:"14px"}}>
            <div style={{fontFamily:T.serif,fontSize:17,color:T.ink,marginBottom:4}}>{preview.title}</div>
            <div style={{fontSize:12,color:T.ink3,marginBottom:8}}>Hook {preview.hook} · {preview.weight} · ~{preview.yardage} yds</div>
            {preview.aiNote&&<div style={{background:T.sageLt,borderRadius:8,padding:"8px 12px",marginBottom:12,display:"flex",gap:8}}><span>🤖</span><span style={{fontSize:12,color:T.sage}}>{preview.aiNote}</span></div>}
            <Btn onClick={()=>onSave({id:Date.now(),photo:imgSrc||PILL[0],rating:0,skeins:4,skeinYards:200,gauge:{stitches:16,rows:20,size:4},dimensions:{width:50,height:60},...preview})}>Save AI Pattern</Btn>
...
const AddPatternModal = ({onClose, onSave, isPro, patternCount}) => {
  ...
  const METHODS = [
    ...
    {key:"url",     icon:"🔗", label:"Import URL",      sub:"Paste any pattern link"},
    {key:"pdf",     icon:"📄", label:"PDF / Document",  sub:"Upload & extract"},
    {key:"browser", icon:"🌐", label:"Browse Sites",    sub:"AllFreeCrochet, Drops & more"},
    {key:"photo",   icon:"📸", label:"AI from Photo",   sub:"Gemini recreates the pattern"},
  ];
              ...
              <div style={{display:"flex",flexDirection:"column",gap:10}}>
                {METHODS.map(m=>(
                  <div key={m.key} className="method-card" onClick={()=>setMethod(m.key)}
                    style={{display:"flex",alignItems:"center",gap:14,padding:"14px 16px",background:T.linen,border:`1.5px solid ${T.border}`,borderRadius:14,cursor:"pointer",transition:"all .15s"}}>
                    <div style={{width:44,height:44,borderRadius:12,background:T.terraLt,display:"flex",alignItems:"center",justifyContent:"center",fontSize:20,flexShrink:0}}>
                      {m.icon}
                    </div>
                    <div style={{flex:1}}>
      ...
      <div style={{padding:"12px 16px 24px"}}>
        {isPro ? (
          ...
          <div style={{background:`linear-gradient(135deg,${T.terra},#8B3A22)`,borderRadius:12,padding:"14px"}}>
            ...
            <div style={{fontSize:11,color:"rgba(255,255,255,.75)",lineHeight:1.5,marginBottom:10}}>Unlimited patterns, all imports, AI recreation, cloud sync.</div>
        ...
        <div style={{padding:"14px 18px 36px"}}>
          {isPro ? (
            ...
            <div style={{background:`linear-gradient(135deg,${T.terra},#8B3A22)`,borderRadius:14,padding:"14px 16px"}}>
              ...
              <div style={{fontSize:11,color:"rgba(255,255,255,.8)",lineHeight:1.5,marginBottom:10}}>Unlimited patterns, all imports, AI recreation.</div>
    ...
    <div style={{minHeight:"100vh",background:T.bg,display:"flex",flexDirection:"column",maxWidth:500,margin:"0 auto"}}>
      ...
      <div style={{background:T.surface,padding:"28px 22px 48px",display:"flex",flexDirection:"column",gap:12}}>
        ...
        <div style={{borderTop:`1px solid ${T.border}`,paddingTop:12,marginTop:4}}>
          <div style={{fontSize:10,color:T.ink3,textTransform:"uppercase",letterSpacing:".1em",textAlign:"center",marginBottom:10}}>Dev / Test Access</div>
          <Btn variant="sage" onClick={onEnterAsPro}>🔑 Enter as Admin — Pro unlocked</Btn>
          <div style={{fontSize:10,color:T.ink3,textAlign:"center",marginTop:6}}>All features active · No pattern cap · Remove before production</div>
        </div>
...
/* ── SHELF CARD with mobile-friendly scroll snap ── */
...
/* ══════════════════════════════════════════════════════════════════════════
   COLLECTION VIEW — shared between mobile and desktop
══════════════════════════════════════════════════════════════════════════ */
const CollectionView = ({patterns, cat, setCat, search, setSearch, openDetail, onAddPattern, isPro, tier}) => {
  ...
  return (
    <>
      ...
      <div style={{display:"flex",gap:7,overflowX:"auto",padding:"0 18px 16px",WebkitOverflowScrolling:"touch"}}>
        {CATS.map(c=>(
      ...
      {/* Free tier usage bar */}
      {!isPro&&(
        <div style={{margin:"0 18px 16px",background:T.linen,borderRadius:12,padding:"10px 14px",border:`1px solid ${T.border}`,display:"flex",alignItems:"center",gap:12}}>
          <div style={{flex:1}}>
      ...
      {/* Pattern grid */}
      <div className="pattern-grid" style={{padding:"0 18px 120px"}}>
        {filtered.length===0
...
export default function StitchBox() {
  ...
  const [isPro,setIsPro]       = useState(false); // wire to Supabase user.profile.is_pro
  ...
  // Detail view — full screen on all devices
  ...
  const openAddModal = () => {
    if(tier.atCap) { setShowPaywall(true); return; }
    setAddOpen(true);
  };
  ...
  if(isDesktop) return (
    <div style={{display:"flex",minHeight:"100vh",background:T.bg,fontFamily:T.sans}}>
      ...
      <div style={{flex:1,minWidth:0,overflowY:"auto"}}>
        {/* Desktop top bar */}
        <div style={{background:T.surface,borderBottom:`1px solid ${T.border}`,padding:"0 32px",height:64,display:"flex",justifyContent:"space-between",alignItems:"center",position:"sticky",top:0,zIndex:20}}>
        ...
        <div style={{maxWidth:1200,margin:"0 auto",padding:"0 32px"}}>
          {view==="collection"&&(
            <CollectionView
              patterns={patterns} cat={cat} setCat={setCat}
              search={search} setSearch={setSearch}
              openDetail={openDetail} onAddPattern={openAddModal}
              isPro={isPro} tier={tier}
            />
          )}
          {view==="wip"&&(
