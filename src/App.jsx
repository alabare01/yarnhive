...
/* ─── VERSION ────────────────────────────────────────────────────────────── */
const APP_VERSION = "v1.0 — Mar 19 2026";
...
const BrowseSitesView = ({onSavePattern}) => {
  ...
  const doImport = async () => {
    ...
    setImportErr(null);
    try {
      const prompt = `You are a crochet pattern parser. Fetch and parse the crochet pattern at this URL. Return ONLY valid JSON matching this schema exactly:
{"title":"","source":"","cat":"Blankets or Wearables or Accessories or Amigurumi or Home Décor or Uncategorized","hook":"","weight":"","yardage":0,"notes":"","materials":[{"id":1,"name":"","amount":"","yardage":0}],"rows":[{"id":1,"text":"","done":false,"note":""}]}
Extract every row, round, and step. Each rows entry = one instruction. URL: ${importUrl.trim()}`;
      ...
      const rows = (parsed.rows||[]).map((r,i)=>({id:Date.now()+i,text:r.text,done:false,note:""}));
      onSavePattern&&onSavePattern({id:Date.now(),photo:PILL[Math.floor(Math.random()*PILL.length)],rating:0,skeins:0,skeinYards:200,gauge:{stitches:12,rows:16,size:4},dimensions:{width:50,height:60},...parsed,rows});
      setActiveSite(null);
      setImportUrl("");
    } catch(e) {
          ...
          <div key={s.name} style={{background:T.surface,borderRadius:16,overflow:"hidden",border:`1px solid ${T.border}`,boxShadow:"0 2px 8px rgba(28,23,20,.05)"}}>
            <div style={{height:110,position:"relative",overflow:"hidden"}}>
              ...
              <div style={{position:"absolute",bottom:10,left:12,right:12,display:"flex",justifyContent:"space-between",alignItems:"flex-end"}}>
                <div style={{fontFamily:T.serif,fontSize:17,fontWeight:700,color:"#fff"}}>{s.name}</div>
                <div style={{background:s.free?"rgba(92,122,94,.9)":"rgba(184,90,60,.9)",borderRadius:7,padding:"3px 8px",fontSize:10,fontWeight:700,color:"#fff"}}>{s.free?"FREE":"FREE + PAID"}</div>
              </div>
            </div>
            <div style={{padding:"12px 14px"}}>
              <div style={{fontSize:12,color:T.ink2,lineHeight:1.6,marginBottom:8}}>{s.desc}</div>
              <div style={{display:"flex",flexWrap:"wrap",gap:4,marginBottom:10}}>
                {s.tags.map(t=><div key={t} style={{background:T.linen,borderRadius:99,padding:"2px 9px",fontSize:10,color:T.ink2,border:`1px solid ${T.border}`}}>{t}</div>)}
              </div>
              <button onClick={()=>setActiveSite(s)} style={{width:"100%",background:`linear-gradient(135deg,${T.terra},#8B3A22)`,color:"#fff",border:"none",borderRadius:10,padding:"10px",fontSize:13,fontWeight:700,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",gap:8}}>
                <span>🌐</span> Browse {s.name}
              </button>
            </div>
          </div>
        ))}
      ...
      <div className="su" onClick={e=>e.stopPropagation()}
        ...
        {shared ? (
          <div style={{textAlign:"center",padding:"16px 0"}}>
            ...
            <div style={{fontFamily:T.serif,fontSize:18,color:T.ink,marginBottom:4}}>Shared!</div>
            <div style={{fontSize:13,color:T.ink3,marginBottom:16}}>Your progress is out in the world.</div>
            <Btn onClick={onClose} variant="secondary">Done</Btn>
          </div>
        ) : (
          <>
