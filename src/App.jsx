...
/* ─── VERSION ────────────────────────────────────────────────────────────── */
const APP_VERSION = "v0.7 — Mar 19 2026";
      ...
      <div style={{flex:1,overflowY:"auto",padding:"4px 20px 36px",maxWidth:isDesktop?760:undefined,margin:isDesktop?"0 auto":undefined,width:"100%"}}>
        {tab==="rows"&&<>
          {(() => {
            ...
            return (
              <div key={r.id}>
                <div onClick={e=>{if(e.target.tagName==="A"||e.target.closest("a"))return; toggle(r.id);}} style={{display:"flex",gap:13,alignItems:"flex-start",cursor:"pointer",background:isCurrent?"rgba(184,90,60,.04)":"transparent",margin:"0 -8px",padding:"14px 8px",borderRadius:isCurrent?8:0,borderBottom:`1px solid ${T.border}`}}>
