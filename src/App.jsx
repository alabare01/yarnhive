...
/* ─── VERSION ────────────────────────────────────────────────────────────── */
const APP_VERSION = "v0.8 — Mar 19 2026";
...
const URLImportForm = ({onSave}) => {
  ...
  const doImport = async () => {
    ...
    try {
      // Use Gemini to fetch and parse the pattern page
      const prompt = `You are a crochet pattern parser. I will give you a URL. Fetch the page content and extract the crochet pattern. Return ONLY a JSON object with this exact schema — no explanation, no markdown, JSON only:

{
  "title": "pattern name",
  "source": "domain name only e.g. allfreecrochet.com",
  "cat": "one of: Blankets, Wearables, Accessories, Amigurumi, Home Décor, Uncategorized",
  "hook": "hook size e.g. 5.0mm or empty string",
  "weight": "yarn weight e.g. Worsted or empty string",
  "yardage": 0,
  "notes": "brief description of the pattern",
  "materials": [
    {"id": 1, "name": "material name", "amount": "amount", "yardage": 0}
  ],
  "rows": [
    {"id": 1, "text": "exact row or step instruction", "done": false, "note": ""}
  ]
}

Extract every single row, round, step, and instruction from the pattern. Each row object should contain one instruction. Include foundation chain, all rounds/rows, finishing instructions. The rows array should give someone everything they need to make this pattern step by step.

URL to parse: ${url.trim()}`;
      ...
      console.error(err);
      setError("Couldn't read that pattern. Try a different URL or use Manual Entry to add it yourself.");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div style={{paddingBottom:8}}>
      ...
      {error&&(
        <div style={{background:"#FFF0EE",borderRadius:12,padding:"14px 16px",marginBottom:14,border:"1px solid #F5C6BB"}}>
          <div style={{fontSize:13,color:"#C0392B",fontWeight:600,marginBottom:4}}>Couldn't read this URL</div>
          <div style={{fontSize:12,color:T.ink2,lineHeight:1.6}}>{error}</div>
        </div>
      )}
        ...
        <div className="fu" style={{background:T.linen,borderRadius:16,overflow:"hidden",border:`1px solid ${T.border}`}}>
          ...
          <div style={{padding:"14px"}}>
            ...
            {preview.rows?.length>0&&(
              <div style={{background:T.surface,borderRadius:10,padding:"10px 12px",marginBottom:12,maxHeight:160,overflowY:"auto",border:`1px solid ${T.border}`}}>
                <div style={{fontSize:10,color:T.ink3,textTransform:"uppercase",letterSpacing:".07em",marginBottom:8,fontWeight:600}}>Preview — {preview.rows.length} steps</div>
                {preview.rows.slice(0,5).map((r,i)=>(
                  <div key={i} style={{fontSize:12,color:T.ink2,padding:"4px 0",borderBottom:i<4?`1px solid ${T.border}`:"none",lineHeight:1.5}}>{r.text}</div>
                ))}
                {preview.rows.length>5&&<div style={{fontSize:11,color:T.ink3,marginTop:6}}>+{preview.rows.length-5} more steps…</div>}
              </div>
            )}
      ...
      <div style={{marginTop:14,background:T.linen,borderRadius:12,padding:"12px 14px"}}>
        ...
        <div style={{fontSize:12,color:T.ink2,lineHeight:1.8}}>AllFreeCrochet · Drops Design · Yarnspirations · LoveCrafts · Sarah Maker · WordPress pattern blogs</div>
...
const Auth = ({onEnter, onEnterAsPro}) => {
  ...
  if(isDesktop) {
    ...
    const showForm = isSignup || isSignin;
    return (
          ...
          <div style={{position:"absolute",bottom:0,left:0,right:0,padding:"0 52px 56px"}}>
            ...
            <div style={{marginTop:32,display:"flex",gap:16}}>
              {["Pattern tracking","Gauge calculator","Snap to Pattern","Yarn stash"].map(f=>(
                <div key={f} style={{background:"rgba(255,255,255,.12)",backdropFilter:"blur(8px)",borderRadius:10,padding:"8px 14px",fontSize:12,color:"rgba(255,255,255,.85)",fontWeight:500}}>{f}</div>
              ))}
            </div>
          </div>
        </div>
            ...
            {!showForm&&(
              <>
                ...
                <div style={{marginTop:20,display:"flex",gap:20}}>
                  <div style={{flex:1,background:T.linen,borderRadius:12,padding:"14px 16px",textAlign:"center",border:`1px solid ${T.border}`}}>
                    <div style={{fontFamily:T.serif,fontSize:22,color:T.terra,fontWeight:700}}>Free</div>
                    <div style={{fontSize:11,color:T.ink3,marginTop:3}}>5 patterns · all features</div>
                  </div>
                  <div style={{flex:1,background:`linear-gradient(135deg,${T.terra},#8B3A22)`,borderRadius:12,padding:"14px 16px",textAlign:"center"}}>
                    <div style={{fontFamily:T.serif,fontSize:22,color:"#fff",fontWeight:700}}>Pro</div>
                    <div style={{fontSize:11,color:"rgba(255,255,255,.7)",marginTop:3}}>$9.99/mo · unlimited</div>
                  </div>
                </div>
...
// Renders row text with first-occurrence abbreviation links
const SmartRowText = ({ text, seenAbbr }) => {
  ...
  while ((match = regex.exec(text)) !== null) {
    ...
    if (!seenAbbr.has(raw)) {
      seenAbbr.add(raw);
      parts.push(
        <span key={match.index} style={{position:"relative",display:"inline"}}>
          <button
            data-stitch-link="true"
            onClick={e=>{
              e.preventDefault();
              e.stopPropagation();
            ...
            onMouseEnter={()=>setTooltip(raw)}
            onMouseLeave={()=>setTooltip(null)}
            style={{color:T.terra,textDecoration:"underline",textDecorationStyle:"dotted",textUnderlineOffset:3,fontWeight:600,cursor:"pointer",background:"none",border:"none",padding:0,font:"inherit",fontSize:"inherit",lineHeight:"inherit"}}
          >
            {match[0]}
          </button>
          {tooltip===raw&&(
            <span style={{position:"absolute",bottom:"calc(100% + 6px)",left:0,zIndex:99,background:T.ink,color:"#fff",borderRadius:8,padding:"6px 10px",fontSize:11,whiteSpace:"nowrap",boxShadow:"0 4px 16px rgba(0,0,0,.25)",pointerEvents:"none"}}>
              <span style={{fontWeight:700}}>{raw}</span> — {info.full}
              <span style={{display:"block",fontSize:10,opacity:.7,marginTop:2}}>Tap to watch tutorial ▶</span>
            </span>
          )}
        </span>
      );
    } else {
      // Subsequent occurrence — plain text
      parts.push(match[0]);
    }
...
/* ══════════════════════════════════════════════════════════════════════════
   BROWSE SITES VIEW
══════════════════════════════════════════════════════════════════════════ */
const BrowseSitesView = () => {
  const {isDesktop} = useBreakpoint();
  const SITES = [
    {
      name:"AllFreeCrochet",
      desc:"The largest free crochet pattern library. Thousands of patterns across every category.",
      url:"https://www.allfreecrochet.com",
      tags:["Blankets","Amigurumi","Wearables","Home"],
      free:true, photo:PILL[4],
    },
    {
      name:"Drops Design",
      desc:"Free international patterns from a leading yarn brand. Strong on garments and accessories.",
      url:"https://www.garnstudio.com",
      tags:["Garments","Accessories","Blankets"],
      free:true, photo:PILL[0],
    },
    {
      name:"Yarnspirations",
      desc:"Official home of Caron and Bernat patterns. Great beginner-friendly selection.",
      url:"https://www.yarnspirations.com/collections/crochet-patterns",
      tags:["Beginner","Blankets","Wearables"],
      free:true, photo:PILL[3],
    },
    {
      name:"Sarah Maker",
      desc:"Modern, well-photographed patterns with clear step-by-step instructions.",
      url:"https://sarahmaker.com/crochet-patterns/",
      tags:["Modern","Beginner","Amigurumi"],
      free:true, photo:PILL[2],
    },
    {
      name:"Hopeful Honey",
      desc:"Beloved amigurumi patterns. Best-in-class for whimsical creatures and stuffed toys.",
      url:"https://www.hopefulhoney.com/p/free-crochet-patterns.html",
      tags:["Amigurumi","Toys","Beginner"],
      free:true, photo:PILL[5],
    },
    {
      name:"The Woobles",
      desc:"Amigurumi kits and free beginner tutorials. Excellent stitch guides alongside patterns.",
      url:"https://thewoobles.com/pages/free-crochet-patterns",
      tags:["Amigurumi","Beginner","Tutorials"],
      free:true, photo:PILL[1],
    },
    {
      name:"Ravelry",
      desc:"The world's largest pattern database — 1M+ patterns, free and paid. Log in on your device first, then copy any pattern URL into Smart Import.",
      url:"https://www.ravelry.com/patterns/library#craft=crochet&view=thumbnail",
      tags:["All categories","Free + Paid","Huge library"],
      free:false, photo:PILL[0],
      note:"Log in on your device first, then use Smart Import with the pattern URL.",
    },
    {
      name:"LoveCrafts",
      desc:"Quality free and paid patterns. Strong on modern garments and colourwork.",
      url:"https://www.lovecrafts.com/en-us/l/crochet/crochet-patterns?price=free",
      tags:["Garments","Modern","Free + Paid"],
      free:false, photo:PILL[2],
    },
  ];
  ...
  return (
    <div style={{padding:isDesktop?"24px 0 80px":"16px 18px 100px"}}>
      <div style={{marginBottom:20}}>
        <div style={{fontSize:13,color:T.ink2,lineHeight:1.7}}>Browse pattern sources below. Find something you love, then copy the URL and use <strong>Smart Import</strong> to pull it into your collection.</div>
        <div style={{marginTop:10,background:T.linen,borderRadius:10,padding:"10px 14px",border:`1px solid ${T.border}`,display:"flex",gap:8,alignItems:"flex-start"}}>
          <span style={{fontSize:15,flexShrink:0}}>💡</span>
          <span style={{fontSize:12,color:T.ink2,lineHeight:1.6}}>Tap <strong>Open Site</strong> to browse. Copy a pattern URL. Return here and tap <strong>+ Add Pattern → Smart Import</strong> to save it.</span>
        </div>
      </div>
        ...
        {SITES.map(s=>(
          <div key={s.name} style={{background:T.surface,borderRadius:16,overflow:"hidden",border:`1px solid ${T.border}`,boxShadow:"0 2px 8px rgba(28,23,20,.05)"}}>
            <div style={{height:120,position:"relative",overflow:"hidden"}}>
              ...
              <div style={{position:"absolute",inset:0,background:"linear-gradient(to top,rgba(20,14,10,.75) 0%,transparent 60%)"}}/>
              <div style={{position:"absolute",bottom:12,left:14,right:14,display:"flex",justifyContent:"space-between",alignItems:"flex-end"}}>
                <div style={{fontFamily:T.serif,fontSize:18,fontWeight:700,color:"#fff"}}>{s.name}</div>
                <div style={{background:s.free?"rgba(92,122,94,.85)":"rgba(184,90,60,.85)",backdropFilter:"blur(4px)",borderRadius:8,padding:"3px 8px",fontSize:10,fontWeight:700,color:"#fff"}}>
                  {s.free?"FREE":"FREE + PAID"}
                </div>
              </div>
            </div>
            <div style={{padding:"14px 16px"}}>
              <div style={{fontSize:13,color:T.ink2,lineHeight:1.6,marginBottom:10}}>{s.desc}</div>
              {s.note&&<div style={{background:T.linen,borderRadius:8,padding:"8px 10px",marginBottom:10,fontSize:11,color:T.ink3,lineHeight:1.5}}>ℹ️ {s.note}</div>}
              <div style={{display:"flex",flexWrap:"wrap",gap:5,marginBottom:12}}>
                {s.tags.map(t=><div key={t} style={{background:T.linen,borderRadius:99,padding:"3px 10px",fontSize:11,color:T.ink2,border:`1px solid ${T.border}`}}>{t}</div>)}
              </div>
              <button
                onClick={()=>window.open(s.url,"_blank","noopener,noreferrer")}
                style={{width:"100%",background:`linear-gradient(135deg,${T.terra},#8B3A22)`,color:"#fff",border:"none",borderRadius:10,padding:"11px",fontSize:13,fontWeight:700,cursor:"pointer",boxShadow:"0 3px 12px rgba(184,90,60,.3)",display:"flex",alignItems:"center",justifyContent:"center",gap:8}}
              >
                <span>🌐</span> Open {s.name}
              </button>
            </div>
          </div>
        ))}
          ...
          {(() => {
            ...
            return (
              <div key={r.id}>
                <div
                  onClick={e=>{
                    if(e.target.closest("[data-stitch-link]")) return;
                    toggle(r.id);
                  }}
                  ...
                  <div style={{flex:1}}>
                    ...
                    <div style={{fontSize:14,lineHeight:1.6,color:r.done?T.ink3:T.ink,textDecoration:r.done?"line-through":"none"}}>
                      {r.done ? r.text : <SmartRowText text={r.text} seenAbbr={seenAbbr}/>}
                    </div>
